...
API.prototype.request = function (url, opts, callback) {
  console.log("[Charles] API.prototype.request "+url);
  var options = {};
...
API.prototype.getAccessToken = function (callback) {
  ...
  this.request(url, {dataType: 'json'}, wrapper(function (err, data) {
    if (err) {
      console.log("[Charles] API.prototype.getAccessToken : return "+err);
      return callback(err);
    }
  ...
  console.log("[Charles] API.prototype.getAccessToken : success");
  return this;
};
...
API.prototype.preRequest = function (method, args, retryed) {
  ...
  that.getToken(function (err, token) {
    if (err) {
      console.log("[Charles] API.prototype.preRequest 0");
      return callback(err);
    }
    var accessToken;
    // 有token并且token有效直接调用
    if (token && (accessToken = AccessToken(token.accessToken, token.expireTime)).isValid()) {
      ...
      if (!retryed) {
        var retryHandle = function (err, data, res) {
          ...
          if (data && data.errcode && data.errcode === 40001) {
            console.log("[Charles] API.prototype.preRequest 1");
            return that.preRequest(method, args, true);
          }
          console.log("[Charles] API.prototype.preRequest 2");
          callback(err, data, res);
        };
        // 替换callback
        var newargs = Array.prototype.slice.call(args, 0, -1);
        newargs.push(retryHandle);
        console.log("[Charles] API.prototype.preRequest 3");
        method.apply(that, newargs);
      } else {
        console.log("[Charles] API.prototype.preRequest 4");
      ...
      that.getAccessToken(function (err, token) {
        // 如遇错误，通过回调函数传出
        if (err) {
          console.log("[Charles] API.prototype.preRequest 5");
          return callback(err);
        }
        // 暂时保存token
        that.token = token;
        console.log("[Charles] API.prototype.preRequest ");
        method.apply(that, args);
      });
    }
  });
