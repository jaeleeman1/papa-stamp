...
function show() {
    ...
    printPrev(this.name);
    print('&nbsp;');
    printDateTF(this.name, this.tfDate, 15, 10, 'border-width:1px; border-color:'+ this.tfDateLineCol + '; border-style:solid; background-color:#FFFFFF; text-align:center');
...
function printDateTF(calObj, name, size, maxlength, style) {
    print("<input type='text' class='showDate' name='" + name + "' size='"+ size +"' maxlength='"+ maxlength +"' style='"+ style +"' onFocus='this.select()' onKeyUp='if (event.keyCode == 13) { "+ calObj +".reload(this);this.select(); }'>");
}
...
function printPrev(calObj) {
    print("<a href=\"javascript:" + calObj + ".prev();\"><i class='fa fa-angle-left' aria-hidden='true' style='font-size:3em;margin-bottom:10px'></i></a>");
}

// 다음달 링크
function printNext(calObj) {
    print("<a href=\"javascript:" + calObj + ".next();\"><i class='fa fa-angle-right' aria-hidden='true' style='font-size:3em;margin-bottom:10px'></i></a>");
}
