...
sender.shoppingResSend =  function(req, res) {
    api.sendText('omHN6wbyhFp4du9PD1xKdI6JGdnE', 'HI', function(err) {
    	console.log("message send");
    });
};
sender.shoppingResSendaa =  function(req, res) {
    ...
    function getShoppingToken() {
        console.log('### Get Shopping Token Start ###');

        var accessTokenURL = config.accessTokenURL+"&appid="+config.appID+"&secret="+config.appsecret;
...
function pushShoppingChat(openId, Result) {
    console.log('### PshShoppingChat Start ###');

    var pushChatURL = config.pushChatURL+ACCESS_TOKEN.access_token;
